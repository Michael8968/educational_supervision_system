...
interface BalancedSubmissionListProps {
  districtId: string;
  projectId: string;
}
...
// 义务教育学校类型
const COMPULSORY_SCHOOL_TYPES = ['小学', '初中', '九年一贯制', '完全中学'];

const BalancedSubmissionList: React.FC<BalancedSubmissionListProps> = ({ districtId, projectId }) => {
  ...
  useEffect(() => {
    ...
    const loadSchools = async () => {
      try {
        ...
        // 过滤义务教育学校
        const compulsorySchools = schoolList.filter((s: School) =>
          COMPULSORY_SCHOOL_TYPES.includes(s.schoolType)
        );
        setSchools(compulsorySchools);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // 过滤只显示义务教育学校的填报记录
        const compulsorySubmissions = result.submissions.filter(
          (s) => COMPULSORY_SCHOOL_TYPES.includes(s.schoolType)
        );
        // 重新计算统计数据
        const filteredStats = {
          total: compulsorySubmissions.length,
          draft: compulsorySubmissions.filter((s) => s.status === 'draft').length,
          submitted: compulsorySubmissions.filter((s) => s.status === 'submitted').length,
          approved: compulsorySubmissions.filter((s) => s.status === 'approved').length,
          rejected: compulsorySubmissions.filter((s) => s.status === 'rejected').length,
        };
        setStats(filteredStats);
        setSubmissions(compulsorySubmissions);
      } catch (error) {
  ...
  const columns: ColumnsType<DistrictSubmission> = [
    ...
    },
    {
      ...
      key: 'schoolType',
      width: 110,
      filters: COMPULSORY_SCHOOL_TYPES.map((type) => ({ text: type, value: type })),
      onFilter: (value, record) => record.schoolType === value,
      render: (type: string) => (
        <Tag color={getSchoolTypeColor(type)}>{type}</Tag>
      ),
    },
    {
      title: '采集工具',
...
export default BalancedSubmissionList;
