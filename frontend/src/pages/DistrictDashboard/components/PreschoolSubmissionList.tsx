 ...
 * 普及普惠（幼儿园）填报列表组件
 *
 * 显示学前教育普及普惠督导评估的幼儿园填报情况
 */
import React, { useState, useEffect } from 'react';
import { Table, Tag, Select, Card, Row, Col, Statistic, Spin, Empty, Modal, Descriptions, Button, Space, Input, message, Alert } from 'antd';
...
interface PreschoolSubmissionListProps {
  districtId: string;
  projectId: string;
}
...
const PreschoolSubmissionList: React.FC<PreschoolSubmissionListProps> = ({ districtId, projectId }) => {
  ...
  useEffect(() => {
    ...
    const loadSchools = async () => {
      try {
        const schoolList = await getDistrictSchools(districtId, { schoolType: '幼儿园' });
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // 过滤只显示幼儿园的填报记录
        const kindergartenSubmissions = result.submissions.filter(
          (s) => s.schoolType === '幼儿园'
        );
        // 重新计算统计数据
        const filteredStats = {
          total: kindergartenSubmissions.length,
          draft: kindergartenSubmissions.filter((s) => s.status === 'draft').length,
          submitted: kindergartenSubmissions.filter((s) => s.status === 'submitted').length,
          approved: kindergartenSubmissions.filter((s) => s.status === 'approved').length,
          rejected: kindergartenSubmissions.filter((s) => s.status === 'rejected').length,
        };
        setStats(filteredStats);
        setSubmissions(kindergartenSubmissions);
      } catch (error) {
  ...
  return (
    <div>
      {/* 普及普惠说明 */}
      <Alert
        message="学前教育普及普惠督导评估 - 幼儿园填报"
        description="本页面展示区县内各幼儿园的数据填报情况，用于支撑普及普惠督导评估。"
        type="info"
        showIcon
      ...
      {loading ? (
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total) => `共 ${total} 条幼儿园填报记录`,
          }}
        />
      ) : (
        <Empty description="暂无幼儿园填报记录" />
      )}
...
export default PreschoolSubmissionList;
