...
| 页面名称 | 路径               | 组件文件              | 功能                       | 核心要素                              |
| -------- | ------------------ | --------------------- | -------------------------- | ------------------------------------- |
| 项目列表 | `/home/balanced` | `Project/index.tsx` | 项目创建、发布、配置、删除 | 项目卡片、状态Tag、发布/配置/删除操作 |
...
| 页面         | 截图                                                           |
| ------------ | -------------------------------------------------------------- |
| 学校账号管理 | ![学校账号管理](screenshots/admin/01-学校账号管理.png)           |
| 专家账号管理 | ![专家账号管理](screenshots/admin/02-专家账号管理.png)           |
| 要素库       | ![要素库](screenshots/admin/03-要素库.png)                       |
| 指标体系库   | ![指标体系库](screenshots/admin/04-指标体系库.png)               |
| 工具库       | ![工具库](screenshots/admin/05-工具库.png)                       |
| 项目列表     | ![项目列表](screenshots/admin/06-项目列表-义务教育.png)          |
...
- 项目名称、关联的指标体系名称
- 项目周期（开始/结束日期）
- 项目状态标签：`配置中` | `填报中` | `评审中` | `已中止` | `已完成`
- 附件列表（政策文件、评估标准等）
- 状态操作按钮

**3. 进度概览**
仅在非"配置中"状态显示，展示填报进度统计。

##### Tab 页签功能详解

**Tab 1: 评估对象 (SubmissionSchoolTab)**

| 功能 | 说明 |
|------|------|
| 统计展示 | 区县数量、学校数量、学校类型分布 |
| 选择学校 | 从系统学校库中选择已有学校 |
| 新增学校 | 创建新学校并添加到项目 |
| 批量导入 | 通过 Excel 文件导入学校 |

**Tab 2: 填报账号 (PersonnelTab)**

| 功能 | 说明 |
|------|------|
| 添加人员 | 添加项目管理员、数据采集员 |
| 关联采集员 | 为评估对象（区县/学校）关联数据采集员 |
| 批量导入 | 批量导入人员 |

**Tab 3: 指标体系 (IndicatorTab)**

| 功能 | 说明 |
|------|------|
| 指标查看 | 显示项目关联的指标体系树形结构 |
| 要素关联 | 通过抽屉组件配置指标与要素的关联 |

**Tab 4: 填报任务 (TaskAssignmentTab)**
...
分配流程（分步骤向导）：
1. 选择采集工具 → 自动识别工具范围（区县/学校级别）
2. 选择评估对象 → 仅显示已配置采集员的区县或学校
3. 确认采集员 → 自动展示将分配给的采集员列表
4. 任务配置 → 是否需要审核、审核员选择、问卷访问配置
5. 设置截止日期（可选）

**Tab 5: 专家评估 (ExpertEvaluationTab)**

| 功能 | 说明 |
|------|------|
| 专家分配 | 配置项目评估专家的评估任务分配 |
...
| 数据审核 | ![数据审核](screenshots/project_admin/02-数据审核.png) |
...
| 数据填报首页 | ![数据填报首页](screenshots/data_collector/02-数据填报首页.png)   |
| 问题整改     | ![问题整改](screenshots/data_collector/03-问题整改.png)           |
...
> 对话框截图位于 `docs/screenshots/dialogs/` 目录下

| 对话框名称           | 角色       | 截图                                                                        |
| -------------------- | ---------- | --------------------------------------------------------------------------- |
| 新增专家弹窗         | 系统管理员 | ![新增专家弹窗](screenshots/dialogs/01-新增专家弹窗.png)                      |
| 创建项目弹窗         | 系统管理员 | ![创建项目弹窗](screenshots/dialogs/02-创建项目弹窗.png)                      |
| 新建要素库弹窗       | 系统管理员 | ![新建要素库弹窗](screenshots/dialogs/03-新建要素库弹窗.png)                  |
| 创建评估指标体系弹窗 | 系统管理员 | ![创建评估指标体系弹窗](screenshots/dialogs/04-创建评估指标体系弹窗.png)      |
| 创建数据采集工具弹窗 | 系统管理员 | ![创建数据采集工具弹窗](screenshots/dialogs/05-创建数据采集工具弹窗.png)      |
| 数据审核驳回弹窗     | 项目管理员 | ![数据审核驳回弹窗](screenshots/dialogs/06-数据审核驳回弹窗.png)              |
...
| 权限代码              | 可访问的主要路由                                                 |
| --------------------- | ---------------------------------------------------------------- |
| `canManageSystem`   | `/users/*`, `/home/balanced`, `/home/balanced/elements`, `/home/balanced/indicators`, `/home/balanced/tools` |
