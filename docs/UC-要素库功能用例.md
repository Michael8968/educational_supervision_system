...
| 属性 | 类型 | 必填 | 输入限制 | 说明 |
|------|------|:----:|----------|------|
| 编码 (code) | string | ✓ | 1-50字符，仅字母数字下划线，库内唯一 | 要素唯一标识，如 `student_count` |
| 名称 (name) | string | ✓ | 1-100字符 | 要素显示名称，如"在校学生数" |
| 数据类型 | enum | ✓ | 固定选项 | 文本、数字、日期、时间、逻辑、数组、文件 |
| 采集级别 | enum | ✓ | 固定选项 | school（学校级）、district（区县级）、auto（自动判断） |
| 计算级别 | enum | | 固定选项 | 派生要素的计算范围：school（学校级）、district（区县级汇总） |
| 关联工具 | ref | 条件 | 下拉选择已发布工具 | 基础要素必填，关联的采集工具ID |
| 关联字段 | ref | 条件 | 下拉选择工具字段 | 基础要素必填，关联的表单字段ID |
| 计算公式 | string | 条件 | 1-500字符，有效公式语法 | 派生要素必填，计算公式表达式 |
| 数据来源说明 | string | | 0-200字符 | 描述数据的具体来源，如"学籍系统导入" |
| 聚合配置 | object | | JSON格式 | 多填报数据的汇总配置（JSON格式） |

### 聚合配置

聚合配置用于处理同一评估对象有多份填报数据时的汇总方式：
...
```json
{
  ...
  "scope": {
    "level": "all",
    "filter": { "field": "school_type", "operator": "eq", "value": "primary" }
  },
  "overwrite": false
}
```
...
| method | enum | 聚合方式（见下表） |
| scope.level | enum | 聚合范围：all（全部）、district（按区县）、school_type（按学校类型）、custom（自定义） |
| scope.filter | object | 可选的过滤条件 |
...
│ │    专任教师数            基础要素  数字   │ │                               │
│ │    采集级别: 学校级      关联工具: 学校... │ │  数据类型                     │
│ │                          [✏️] [🗑️]      │ │                               │
│ └──────────────────────────────────────────┘ │  [数字 ▼]                     │
│ ┌──────────────────────────────────────────┐ │                               │
│ │ 🔢 student_teacher_ratio                 │ │  采集级别                     │
│ │    师生比                派生要素  数字   │ │  ○ 学校级  ○ 区县级  ○ 自动  │
│ │    公式: teacher_count / student_count   │ │  ── 基础要素配置 ──           │
│ │                          [✏️] [🗑️]      │ │                               │
│ └──────────────────────────────────────────┘ │  关联采集工具                 │
│                                              │  [学校基本信息采集表 ▼]       │
│                                              │                               │
...
| 要素被佐证材料引用 | 提示"该要素被以下佐证材料引用，无法删除：{材料列表}" |
