...
```
┌─ 采集工具库 ──────────────────────────────────────────────────────────────────┐
│  [搜索工具________________🔍]  [全部状态 ▼]  [全部类型 ▼]  [+ 创建采集工具]  │
├───────────────────────────────────────────────────────────────────────────────┤
│ ┌─────────────────────────────────────────────────────────────────────────┐   │
│ │ 学校基本信息采集表  [表单] [填报对象: 学校]                指标数: 15 草稿│   │
│ │ 采集学校基本信息，包括学校名称、地址、办学类型等基础数据                 │   │
│ │ 创建时间: 2024-06-01  创建人: admin  更新时间: 2025-12-20  更新人: admin │   │
│ │                                                                         │   │
│ │             [◉ 工具信息] [✏️ 编辑工具] [发布] [🗑️ 删除]                 │   │
│ └─────────────────────────────────────────────────────────────────────────┘   │
│ ┌─────────────────────────────────────────────────────────────────────────┐   │
│ │ 家长满意度调查问卷  [问卷] [填报对象: 家长]                      已发布  │   │
│ │ 对家长进行教育满意度调查，包括教学质量、校园安全、家校沟通等维度         │   │
│ │ 创建时间: 2024-05-15  创建人: admin  更新时间: 2025-12-18  更新人: admin │   │
│ │                                                                         │   │
│ │             [◉ 工具信息] [✏️ 编辑工具] [🔗 生成链接] [取消发布]         │   │
│ └─────────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────────────────────────────────────────────┘
```
...
| 状态 | 类型 | 可用操作 |
|------|------|----------|
| 草稿/编辑中 | 表单 | 工具信息、编辑工具、发布、删除 |
| 草稿/编辑中 | 问卷 | 工具信息、编辑工具、发布、删除 |
| 已发布 | 表单 | 工具信息、编辑工具、取消发布 |
| 已发布 | 问卷 | 工具信息、编辑工具、生成链接、取消发布 |
...
| 编辑工具 | 点击"编辑工具"按钮 | 表单→跳转编辑器，问卷→跳转第三方 |
| 生成链接 | 点击"生成链接"按钮（问卷） | 弹出链接生成弹窗 |
| 发布 | 点击"发布"按钮 | 直接发布 |
...
| 用例编号 | UC-TOOL-01 |
| 用例名称 | 创建采集工具 |
| 用例简述 | 创建新的采集工具，设置工具类型、名称、填报对象等基础信息，创建后根据类型进入编辑或跳转第三方系统 |
| 参与者 | 系统管理员 |
| 前置条件 | 用户已登录且具有 canManageSystem 权限 |
| 后置条件 | 工具创建成功，状态为"草稿"，表单类型跳转编辑器，问卷类型跳转第三方 |

*输入字段：*
| 字段 | 类型 | 必填 | 限制 | 说明 |
|------|------|:----:|------|------|
| 工具类型 | enum | ✓ | 表单 \| 问卷 | 决定编辑方式和数据采集方式 |
...
| 条件控制 | 条件显示 (conditionalDisplay) | 全部 | 根据其他字段值控制显示 |
| | 动态标签 (labelWhen) | 全部 | 根据条件动态切换标签 |
| 评价依据 | 映射 (mappings) | 全部 | 关联数据指标或要素 |
...
| 用例编号 | UC-TOOL-03 |
| 用例名称 | 生成问卷访问链接 |
| 用例简述 | 为问卷类型工具批量生成学校访问链接，支持设置目标受众、链接类型和有效期，可批量复制或导出 |
| 参与者 | 系统管理员、项目管理员 |
| 前置条件 | 工具类型为"问卷"，状态为"已发布" |
| 后置条件 | 生成访问链接列表 |
...
┌─ 生成结果 ──────────────────────────────────────────────────────────┐
│ 已生成 3 个访问链接                      [批量复制] [导出CSV]       │
├─────────────────────────────────────────────────────────────────────┤
│ ┌─────────────────────────────────────────────────────────────────┐│
│ │ 第一小学                                                    [复制]││
│ │ https://survey.example.com/s/abc123?token=xyz                   ││
│ │ 有效期至: 2025-01-05 12:00                                      ││
│ └─────────────────────────────────────────────────────────────────┘│
│ ┌─────────────────────────────────────────────────────────────────┐│
│ │ 第二小学                                                    [复制]││
│ │ https://survey.example.com/s/def456?token=uvw                   ││
│ │ 有效期至: 2025-01-05 12:00                                      ││
│ └─────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
```
...
| 项目 | ref | ✓ | 关联的评估项目 |
| 学校 | ref[] | ✓ | 批量选择学校（支持全选） |
| 目标受众 | enum | ✓ | 家长 / 教师 / 学生 / 校长 |
| 链接类型 | enum | ✓ | 普通链接 / 短链接 / 二维码 |
| 有效期 | number | | 小时数，默认168（7天） |
...
2. 选择关联项目
3. 选择目标学校（支持全选）
4. 选择目标受众（家长/教师/学生/校长）
5. 选择链接类型
6. 设置有效期（可选）
7. 点击"生成链接"
8. 显示生成结果列表
9. 可复制单个链接、批量复制或导出CSV
...
| 用例编号 | UC-TOOL-05 |
| 用例名称 | 取消发布采集工具 |
| 用例简述 | 将已发布的采集工具恢复为编辑中状态，项目中已使用的工具副本不受影响 |
| 参与者 | 系统管理员 |
| 前置条件 | 工具状态为"已发布" |
| 后置条件 | 工具状态变为"编辑中" |
...
| 用例编号 | UC-TOOL-06 |
| 用例名称 | 删除采集工具 |
| 用例简述 | 删除未发布的采集工具及其关联数据（表单结构、字段映射等） |
| 参与者 | 系统管理员 |
| 前置条件 | 工具状态为"草稿"或"编辑中" |
| 后置条件 | 工具及关联数据从系统中移除 |
