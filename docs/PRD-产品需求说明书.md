...
| 用例编号 | UC-IND-02 |
| 用例名称 | 构建指标树结构 |
| 用例简述 | 在指标树编辑页面，通过添加、删除、拖拽等操作构建2-3级灵活指标层次结构，系统自动生成规范化的指标编码 |
| 参与者 | 系统管理员 |
...
| 层级 | number | 自动 | 1-3，根据父节点自动计算 | 指标层级（最多3级） |
...
4. **可选**：选中二级指标，点击"添加子指标"创建三级指标（若不添加，二级指标即为叶子节点）
5. 双击或在右侧面板编辑指标名称
6. 拖拽指标节点调整顺序（同级内拖拽）
7. 点击"保存"按钮，系统自动生成/更新编码

> **灵活结构说明**：二级指标可以直接作为叶子节点配置数据指标，无需强制创建三级指标
...
| 用例编号 | UC-IND-03 |
| 用例名称 | 配置叶子指标详情 |
| 用例简述 | 为叶子指标（二级或三级）配置数据指标（含达标阈值）和佐证材料要求，建立与要素库的关联，评分类体系还需设置权重 |
| 参与者 | 系统管理员 |
| 前置条件 | 指标树结构已创建，存在叶子指标（无子节点的二级或三级指标） |
| 后置条件 | 叶子指标的数据指标和佐证材料配置保存成功 |
...
> **注意**：只有叶子节点（无子指标）才能配置数据指标和佐证材料。若需要为某指标添加子指标，需先清空其数据指标配置。
