...
| 阈值 | string | ✓ | 支持格式见下表 | 达标判定标准，显示为标签 |
| 说明 | string | | 0-500字符 | 指标详细说明 |
| 关联要素 | ref | | 选择已发布要素库中的要素 | 数据来源 |

*阈值格式说明：*
| 类型 | 格式 | 示例 | 说明 |
|------|------|------|------|
| 等于 | =数值 | =100%、=100 | 数值=阈值则达标 |
| 大于等于 | >=数值 或 ≥数值 | >=90、≥85% | 数值≥阈值则达标 |
| 小于等于 | <=数值 或 ≤数值 | <=0.50、≤5 | 数值≤阈值则达标 |
| 比例 | 数值:数值 | 1:15、1:19 | 师生比等比例类指标 |
| 区间 | [数值,数值] | [85,100] | 数值在区间内达标 |
