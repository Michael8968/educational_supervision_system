...
| 字段 | 类型 | 必填 | 限制 | 说明 |
|------|------|:----:|------|------|
| 编码 | string | 自动 | 格式：A/A1/A1.1，系统生成 | 指标唯一标识 |
| 名称 | string | ✓ | 1-200字符 | 指标显示名称 |
| 层级 | number | 自动 | 1-3，根据父节点自动计算 | 指标层级 |
| 排序 | number | 自动 | 同级内唯一，支持拖拽调整 | 显示顺序 |

*编码生成规则：*
| 层级 | 格式 | 示例 |
|------|------|------|
| 一级 | 大写字母 | A, B, C, ... Z, AA, AB... |
| 二级 | 一级编码 + 数字 | A1, A2, B1, B2... |
| 三级 | 二级编码 + .数字 | A1.1, A1.2, B2.1... |

*主流程：*
1. 进入指标树编辑页面，左侧显示树形结构，右侧显示属性面板
2. 点击"添加一级指标"，创建根节点，默认名称"新指标"
3. 选中一级指标，点击"添加子指标"创建二级指标
4. 选中二级指标，点击"添加子指标"创建三级指标
5. 双击或在右侧面板编辑指标名称
6. 拖拽指标节点调整顺序（同级内拖拽）
7. 点击"保存"按钮，系统自动生成/更新编码

*异常流程：*
| 异常 | 处理方式 |
|------|----------|
| 三级指标下添加子节点 | 按钮禁用，Tooltip提示"最多支持三级指标" |
| 指标名称为空 | 保存时校验失败，高亮显示问题节点 |
| 删除含子节点的指标 | 弹窗确认"删除后子指标将一并删除，是否继续？" |
| 跨级拖拽 | 不支持，拖拽时视觉提示无效区域 |
...
| 用例名称 | 配置三级指标详情 |
| 参与者 | 系统管理员 |
| 前置条件 | 指标树结构已创建，存在三级指标 |
| 后置条件 | 三级指标的数据指标和佐证材料配置保存成功 |

*数据指标输入字段：*
| 字段 | 类型 | 必填 | 限制 | 说明 |
|------|------|:----:|------|------|
| 编码 | string | ✓ | 1-50字符，体系内唯一 | 数据指标标识 |
| 名称 | string | ✓ | 1-200字符 | 数据指标名称 |
| 阈值 | string | ✓ | 支持格式见下表 | 达标判定标准 |
| 阈值类型 | enum | | single \| range，默认single | 单值或区间 |
| 精度 | number | | 0-4，默认2 | 小数位数 |
| 目标类型 | string | | 1-50字符 | 指标分类标识 |
| 关联要素 | ref | | 选择已发布要素库中的要素 | 数据来源 |

*阈值格式说明：*
| 类型 | 格式 | 示例 | 说明 |
|------|------|------|------|
| 单值-大于等于 | ≥数值 | ≥90、≥1.5 | 数值≥阈值则达标 |
| 单值-小于等于 | ≤数值 | ≤5、≤0.1 | 数值≤阈值则达标 |
| 单值-等于 | =数值 | =100、=1 | 数值=阈值则达标 |
| 比例 | 数值:数值 | 1:15、1:19 | 师生比等比例类指标 |
| 区间 | [数值,数值] | [85,100]、[0.5,1.5] | 数值在区间内达标 |
| 百分比 | 数值% | 90%、≥85% | 百分比类指标 |
...
| 字段 | 类型 | 必填 | 限制 | 说明 |
|------|------|:----:|------|------|
| 权重 | number | ✓ | 0-100，同级权重和=100 | 指标权重百分比 |
...
| 阈值格式错误 | 提示"阈值格式不正确，请参考格式说明" |
| 权重和不等于100 | 提示"同级指标权重之和必须等于100%" |
| 必填数据指标未关联要素 | 警告提示，允许保存但标记为不完整 |
...
| 校验项 | 规则 | 错误提示 |
|--------|------|----------|
| 指标结构 | 至少包含一个完整的三级指标链 | "请至少创建一个完整的三级指标" |
| 指标名称 | 所有指标名称不为空 | "存在未命名的指标：{编码列表}" |
| 数据指标 | 三级指标至少有一个数据指标 | "以下指标缺少数据指标：{编码列表}" |
...
| 检查项 | 说明 |
|--------|------|
| 进行中项目 | 检查是否有"配置中"或"填报中"状态的项目正在使用 |
| 已完成项目 | 已完成项目不受影响，保留历史数据 |
...
| 异常 | 处理方式 |
|------|----------|
| 有进行中项目使用 | 警告"以下项目正在使用该指标体系：{项目列表}，取消发布后新项目将无法选用，是否继续？" |
...
| 用例名称 | 删除指标体系 |
| 参与者 | 系统管理员 |
| 前置条件 | 指标体系状态为"草稿"，未被任何项目引用 |
| 后置条件 | 指标体系及关联数据从系统中移除 |

*删除校验：*
| 校验项 | 规则 | 错误提示 |
|--------|------|----------|
| 状态检查 | 必须为"草稿"状态 | "仅草稿状态的指标体系可删除" |
| 项目引用 | 不能被任何项目引用 | "该指标体系已被以下项目引用：{项目列表}" |
