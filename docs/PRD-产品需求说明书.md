...
| 主流程 | 1. 点击"新建指标体系"按钮<br>2. 填写基础信息（名称、类型、目标、描述）<br>3. 选择指标体系类型（达标类/评分类）<br>4. 添加标签（可选）<br>5. 上传附件（可选）<br>6. 点击"创建"按钮 |
| 后置条件 | 指标体系创建成功，状态为"草稿"，跳转到指标树编辑页面 |
| 异常流程 | 名称重复时提示错误；必填项未填时阻止提交 |
...
| 主流程 | 1. 进入指标树编辑页面<br>2. 点击"添加一级指标"创建根节点<br>3. 选中一级指标，点击"添加子指标"创建二级指标<br>4. 选中二级指标，点击"添加子指标"创建三级指标<br>5. 编辑各指标的名称<br>6. 拖拽调整指标顺序<br>7. 点击"保存"按钮 |
| 后置条件 | 指标树结构保存成功，编码自动生成（A、A1、A1.1格式） |
| 异常流程 | 三级指标下禁止添加子节点；删除指标时级联删除所有子指标 |
...
| 主流程 | 1. 选中三级指标<br>2. 在右侧面板配置数据指标：<br>   - 添加数据指标（编码、名称、阈值）<br>   - 设置阈值类型和精度<br>3. 配置佐证材料：<br>   - 添加材料项（编码、名称、文件类型）<br>   - 设置是否必填<br>4. 设置权重（评分类体系）<br>5. 保存配置 |
| 后置条件 | 三级指标详情保存成功 |
| 异常流程 | 数据指标编码重复时提示；阈值格式错误时校验失败 |
...
| 参与者 | 系统管理员 |
| 前置条件 | 指标体系已创建 |
| 主流程 | 1. 点击"导入"按钮<br>2. 上传 JSON 格式的指标树文件<br>3. 系统解析并预览导入内容<br>4. 确认导入<br>5. 系统自动规范化编码 |
| 后置条件 | 指标树导入成功，替换或追加现有结构 |
| 异常流程 | JSON 格式错误时提示；层级超过三级时截断或报错 |

**UC-IND-05: 发布指标体系**
| 项目 | 内容 |
|------|------|
| 参与者 | 系统管理员 |
| 前置条件 | 指标体系状态为"草稿"或"编辑中"，至少包含一个完整的三级指标 |
| 主流程 | 1. 在指标体系列表点击"发布"按钮<br>2. 确认发布操作<br>3. 系统校验指标完整性<br>4. 发布成功 |
| 后置条件 | 指标体系状态变为"已发布"，可在项目配置中选用 |
| 异常流程 | 无三级指标时阻止发布；校验失败时提示具体问题 |
...
| 参与者 | 系统管理员 |
| 前置条件 | 指标体系状态为"已发布" |
| 主流程 | 1. 点击"取消发布"按钮<br>2. 确认操作 |
| 后置条件 | 指标体系状态变为"编辑中"，不可在新项目中选用 |
| 异常流程 | 如有进行中的项目正在使用该体系，提示警告 |
...
| 参与者 | 系统管理员、项目管理员 |
| 前置条件 | 指标体系存在 |
| 主流程 | 1. 在列表点击"查看详情"<br>2. 弹窗显示基础信息（名称、类型、状态、创建信息）<br>3. 展示指标预览（前5个指标）<br>4. 点击"关闭"或进入编辑 |
| 后置条件 | 无状态变更 |
...
| 主流程 | 1. 点击"删除"按钮<br>2. 确认删除操作<br>3. 系统删除指标体系及关联的所有指标数据 |
| 后置条件 | 指标体系从系统中移除 |
| 异常流程 | 已发布或被项目引用的体系不可删除 |
