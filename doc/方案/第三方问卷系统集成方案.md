...
|                      | `total_satisfied`              | 满意数     | 回调接口                   |
|                      | `total_satisfied_from_parents` | 家长满意数 | 回调接口                   |
...
- 无需 `projectId`：此时工具尚未关联项目，一个工具可被多个项目使用
- 无需 `schoolId`：创建的是全局问卷模板
...
- **无需 `projectId`**：创建工具时还未关联项目，一个工具可被多个项目使用
- **无需 `schoolId`**：管理员创建的是全局问卷模板，所有学校共用
