...
> **最后更新时间**：2025-12-22
> **校对版本**：v2.0
> **校对说明**：本次更新根据实际代码实现对文档进行了全面校对，主要修正了以下内容：
> 1. **data_indicators 表结构**：移除了实际不存在的字段（data_type, unit, calculation_method, collection_frequency），这些属性实际由 elements 表管理
> 2. **TypeScript 类型定义**：更新了前后端的类型定义，与实际代码保持一致
> 3. **FormField 接口**：补充了完整的字段定义，包括数字类型特有属性、映射信息等
> 4. **公式计算机制**：补充了扩展公式计算模式的说明，包括数组操作、聚合函数等高级功能
...
**注意**：早期设计中包含 `data_type`、`unit`、`calculation_method`、`collection_frequency` 等字段，但实际实现中这些属性由 `elements` 表管理，通过 `data_indicator_elements` 关联表建立联系。
...
// 注意：data_type、unit、calculation_method、collection_frequency 等属性
// 实际存储在 elements 表中，通过 data_indicator_elements 关联表与数据指标建立联系
...
#### 数据指标相关 (`frontend/src/services/indicatorService.ts`)
...
2. 从提交数据中提取元素值：
   - 对于基础公式计算：仅保留**可解析为数值**的字段（文本/布尔等会被忽略为不可计算）
   - 对于扩展公式计算：保留原始值（包括数组、对象等复杂类型）
...
4. 公式计算（支持两种模式）：
   ...
   **a) 基础公式计算**（传统模式）：
   ...
   - 示例：`E001 / E002 * 100` 计算生师比百分比

   **b) 扩展公式计算**（增强模式）：
   - 当公式包含扩展函数关键字时自动启用（如 `SUM`, `AVG`, `COUNT`, `FILTER` 等）
   - 支持数组操作、条件筛选、聚合函数等高级功能
   - 使用 `aggregationService` 进行计算，支持复杂数据结构
   - 布尔类型结果会转换为 1/0 数值
   - 示例：`SUM(E001) / COUNT(E001)` 对数组求和后计算平均值

5. 若某数据指标已有"直接映射"的值，则**公式结果不会覆盖**（直接映射优先级更高）
6. 写入 `school_indicator_data` 并做阈值达标判定

**实现位置**：`backend/routes/submissions.js` 第119-341行 `syncSubmissionIndicators` 函数
